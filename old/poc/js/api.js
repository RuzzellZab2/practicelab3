...
export function isLessonPassed(studentId, lessonId) {
  ...
  const allStepsDone = steps.length > 0 && steps.every((s) => isStepDone(studentId, s.id));
  if (lesson.has_work) {
...
export const api = {
  ...
  async listCoursesForAuthor(authorId) {
    ...
    return db.courses
      ...
      .map((c) => {
        ...
        const reviewer = link ? findUser(link.reviewer_id) : null;
        return {
          course: { ...c },
  ...
  async getAuthorCourseDetail(courseId) {
    await delay(200);
    const c = findCourse(courseId);
    if (!c) throw new Error('Курс не найден');
    const lessons = db.lessons
      ...
      .map((l) => ({
        ...l,
        stepCount: db.steps.filter((s) => s.lesson_id === l.id).length,
      }));
    const link = db.courseReviewers.find((r) => r.course_id === courseId);
    const reviewer = link ? findUser(link.reviewer_id) : null;
    return { course: { ...c }, lessons, reviewer: reviewer ? { ...reviewer } : null };
  },
  ...
  async assignReviewer(courseId, reviewerId) {
    ...
    const r = findUser(reviewerId);
    if (!r || r.role !== 'reviewer') throw new Error('Ревьюер не найден');
  ...
  async listWorksOnReview(reviewerId) {
    ...
    return db.works
      ...
      .map((w) => {
        const lesson = findLesson(w.lesson_id);
        const course = lesson ? findCourse(lesson.course_id) : null;
        const student = findUser(w.student_id);
        return {
          work: { ...w },
          lesson: lesson ? { ...lesson } : null,
          course: course ? { ...course } : null,
